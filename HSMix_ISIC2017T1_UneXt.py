...
from networks.A1218_UNeXt_binary_base import *
...
def make_print_to_file(path='./'):
    ...
    sys.stdout = Logger(
        fileName + 'A0313_ISIC2017T1_UneXt_SuperpixelCutMixMixup_2Mixup_2Aug.log', path=path)
...
def main():
    ...
    model_name = './checkpoints/A0313_ISIC2017T1_UneXt_SuperpixelCutMixMixup_2Mixup_2Aug.pth'
    if len(opts.gpu_ids) > 1:
        ...
        if opts.RESUME:
            ...
            model = UNext(num_classes=1).cuda()
            model = model.cuda()
    ...
    results_file_name = results_out_dir + tm + 'A0313_ISIC2017T1_UneXt_SuperpixelCutMixMixup_2Mixup_2Aug_results.txt'
        ...
        for i, (samples, _) in tqdm(enumerate(train_loader)):
            ...
            if r < opts.cutmix_prob:
                ...
                images_SuperPCutMix, images_SuperPMixup, labels_SuperPCutMix, labels_SuperPMixup = \
                    SuperpixelMixup_LambdaMask(images, labels, opts.N_min, opts.N_max, Beta_mixup=opts.Beta_mixup,
