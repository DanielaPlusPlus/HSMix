...
def make_print_to_file(path='./'):
    ...
    sys.stdout = Logger(
        fileName + 'A0313_GlaS_UNet_SuperpixelCutMixMixup_2Mixup_2Aug.log', path=path)
make_print_to_file(path='./logs')

def get_argparser():
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset", type=str, default='GlaS',
                        choices=['GlaS', 'MoNuSeg', 'ISIC2017T1', 'ISIC2016B3'], help='Name of dataset')
...
def main():
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(filelists)):
        ...
        model_name = './checkpoints/A0313_GlaS_UNet_SuperpixelCutMixMixup_2Mixup_2Aug_%sFold.pth' %  (fold)
        if len(opts.gpu_ids) > 1:
            if opts.RESUME:
                model = SegNet.U_Net(img_ch=3)
        ...
        results_file_name = results_out_dir + tm + 'A0313_GlaS_UNet_SuperpixelCutMixMixup_2Mixup_2Aug_%sFold_results.txt' %  (fold)
