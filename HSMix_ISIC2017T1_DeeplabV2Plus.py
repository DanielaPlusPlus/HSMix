...
def make_print_to_file(path='./'):
    ...
    sys.stdout = Logger(
        fileName + 'A0313A_ISIC2017T1_DeeplabV3Plus_2Mixup_2Aug_Saliency_05.log', path=path)
...
def get_argparser():
    ...
    available_models = sorted(name for name in DeepLab_network.modeling_binary_base.__dict__ if name.islower() and \
                              not (name.startswith("__") or name.startswith('_')) and callable(
                              DeepLab_network.modeling_binary_base.__dict__[name])
                              )
...
def main():
    ...
    model_name = './checkpoints/A0313A_ISIC2017T1_DeeplabV3Plus_2Mixup_2Aug_Saliency_05.pth'
    if len(opts.gpu_ids) > 1:
        if opts.RESUME:
            model = DeepLab_network.modeling_binary_base.__dict__[opts.model](output_stride=opts.output_stride)
            if opts.separable_conv and 'plus' in opts.model:
                DeepLab_network.convert_to_separable_conv(model.classifier)
    ...
    results_file_name = results_out_dir + tm + 'A0313A_ISIC2017T1_DeeplabV3Plus_2Mixup_2Aug_Saliency_05_results.txt'
